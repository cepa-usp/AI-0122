...
var LOCAL_STORAGE_KEY = "AI-0122_v2011.08.08_1";
...
function reset () {
  ...
  memento.answers = ["", "", "", "", "", "", "", ""];
  memento.A = [movie.getAPointXCoordinate(), movie.getAPointYCoordinate()];
  ...
  memento.show = [];
  memento.showVersor = false;
  memento.count = 0;
  memento.restore = [];
  memento.interactive = [true, true, true, true];
...
function fetch () {
  ...
  ans.learner = "";
  ans.show = [];
  ans.showVersor = false;
  ans.count = 0;
  ans.restore = [];
  ans.interactive = [true, true, true, true];
...
// Oculta/exibe a janela de atividade
function toggleSidebar () {
	var posicao = parseInt($('#slidebottom').css('right'),10);
	var newPos = -$('#slidebottom').outerWidth() + 20;
	var oldPos = 0;
	...
	if (posicao == 0) {
		...
		$('.borda').removeClass('fechada').addClass('aberta');
	}
	else {
		$('#slidebottom').animate({
			right: oldPos
		});
		$('.borda').removeClass('aberta').addClass('fechada');
	}
}
